...
# main file for the embedded topic model by:
...
def etm_training(args, embedding_path):
    ...
    from etm import ETM
    from utils_etm import nearest_neighbors, get_topic_coherence, get_topic_diversity
    ...
    vect_path = os.path.join(args.data_path.split('/')[0], embedding_path)
    word2vec = KeyedVectors.load(vect_path)
    embedding_vocab = word2vec.key_to_index
    keys = embedding_vocab.keys()
    vectors = {}
    for word in vocab:
        if word in keys:
